...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string marca,int& qtd);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string marca){
    int qtd=0;
    levantamentoAux(raiz,marca,qtd);
    return qtd;
}


void avl::levantamentoAux(noh* umNoh, string marca,int& qtd){
    if(umNoh != NULL){
        levantamentoAux(umNoh->esq,marca,qtd);
        if(umNoh->elemento.marca == marca)
            qtd += umNoh->elemento.quantidadeDisponivel;
        levantamentoAux(umNoh->dir,marca,qtd);
    }
}
