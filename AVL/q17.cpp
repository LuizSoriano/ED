...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string local,int& qtd);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string local){
    int qtd=0;
    levantamentoAux(raiz,local,qtd);
    return qtd;
}

void avl::levantamentoAux(noh* umNoh, string local,int& qtd){
    if(umNoh != NULL){
        levantamentoAux(umNoh->esq,local,qtd);
        if(umNoh->elemento.localizacao == local)
            qtd += umNoh->elemento.quantidadeDisponivel;
        levantamentoAux(umNoh->dir,local,qtd);
    }
}
