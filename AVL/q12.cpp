...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* atual, unsigned int &maior,unsigned int qtd,unsigned int &ano);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
unsigned avl::levantamento(){
    ...
    unsigned int ano = raiz->elemento.ano;
    levantamentoAux(raiz,maior,raiz->elemento.qtdadeFilmesApresentados,ano);
    return ano;
}

void avl::levantamentoAux(noh* atual, unsigned int &maior, unsigned int qtd, unsigned int &ano) {
    if(atual != NULL){
        levantamentoAux(atual->esq,maior,qtd,ano);
        if(atual->elemento.qtdadeFilmesApresentados > maior){
            maior = atual->elemento.qtdadeFilmesApresentados;
            ano = atual->elemento.ano;
        }
        else if(atual->elemento.qtdadeFilmesApresentados == maior){
            if(ano > atual->elemento.ano)
                ano = atual->elemento.ano;
        }
        levantamentoAux(atual->dir,maior,qtd,ano);
    }
}
