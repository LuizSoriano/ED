...
class avl {
    ...
    private:
        ...
        void percorreEmOrdemAux(noh* atual, int &nivel,string marca,short ano);
...
void avl::percorreEmOrdemAux(noh* atual, int &nivel, string marca, short ano) {
    if(atual != NULL){
        percorreEmOrdemAux(atual->esq,nivel,marca,ano);
        if(atual->elemento.marca == marca and atual->elemento.anoFabricacao >= ano)
            nivel++;
        percorreEmOrdemAux(atual->dir,nivel,marca,ano);
    }
}
...
int avl::levantamento(string marca,short ano){
    noh* umNoh = raiz;
    int qtd = 0;
    percorreEmOrdemAux(umNoh,qtd, marca,ano);
    return qtd;
}
        ...
        try {
            ...
            switch (operacao) {
                case 'i': // Inserir recursivamente
                    ...
                    cin >> umDado.chave >> umDado.modelo >> umDado.marca >> umDado.anoFabricacao;
