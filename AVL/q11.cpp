...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* atual,int &cont, string cult, string marca);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string cult,string marca){
    if(raiz != NULL){
        noh* atual = raiz;
        int cont = 0;
        levantamentoAux(atual,cont,cult,marca);
        return cont;
    }
    return 0;
}

void avl::levantamentoAux(noh* atual,int &cont, string cult, string marca){
    if(atual != NULL){
        levantamentoAux(atual->esq,cont,cult,marca);
        if(atual->elemento.nomeCultivar == cult and atual->elemento.marcaSemente == marca)
            cont += atual->elemento.quantidadeDisponivel;
        levantamentoAux(atual->dir,cont,cult,marca);
    }
}
